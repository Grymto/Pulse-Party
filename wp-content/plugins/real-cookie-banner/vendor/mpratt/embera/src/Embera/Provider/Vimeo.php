...
class Vimeo extends ProviderAdapter implements ProviderInterface
{
    ...
    /** inline {@inheritdoc} */
    protected $allowedParams = ['autopause', 'autopip', 'autoplay', 'background', 'byline', 'color', 'controls', 'dnt', 'keyboard', 'loop', 'muted', 'pip', 'playsinline', 'portrait', 'quality', 'responsive', 'speed', 'texttrack', 'title', 'transparent'];
    /** inline {@inheritdoc} */
    protected $httpsSupport = \true;
    /** inline {@inheritdoc} */
    public function validateUrl(Url $url)
    {
        return (bool) (\preg_match('~vimeo\\.com/(?:[0-9]{5,})$~i', (string) $url) || \preg_match('~vimeo\\.com/channels/(?:[^/]+)/(?:[^/]+)~i', (string) $url) || \preg_match('~vimeo\\.com/event/(?:[^/]+)/(?:[^/]+)~i', (string) $url) || \preg_match('~vimeo\\.com/groups/(?:[^/]+)/videos/(?:[^/]+)~i', (string) $url) || \preg_match('~vimeo\\.com/ondemand/(?:[^/]+)/(?:[^/]+)~i', (string) $url) || \preg_match('~vimeo\\.com/(?:[0-9]{5,})/(?:[0-9a-z]+)$~i', (string) $url) || \preg_match('~vimeo\\.com/video/(?:[^/]+)~i', (string) $url));
    }
    /** inline {@inheritdoc} */
